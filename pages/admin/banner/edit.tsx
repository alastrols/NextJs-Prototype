...
import { getBannerId, editBanner } from "@/services/admin/adminService";
import { productImageURL, getBase64 } from "@/utils/commonUtil";
type Props = {
  banner?: BannerData;
};

const Add = ({ banner }: Props) => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const [dateSend, setDateSend] = React.useState(`${banner?.post_date}`);
  const initialValues: any = {
    banner_id: `${banner?.banner_id}`,
    banner_name: `${banner?.banner_name}`,
    banner: `${banner?.banner}`,
    post_date: `${banner?.post_date}`,
    ...
    if (values.file_obj) {
      ...
      return (
        <Image
          objectFit="contain"
          alt="product image"
          src={productImageURL("banner", banner?.banner)}
          width={100}
          height={100}
        />
      );
      ...
      <div>
        ...
        <Formik
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            data.append("banner_id", String(values.banner_id));
